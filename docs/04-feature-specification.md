...
The MVP is the **deterministic pipeline** and the backend that supports it:

> **Log in → create a project → draw a floor plan in the 2D editor → convert it to a 3D scene →
> render it to GLB → edit it in the 3D editor → save it durably**, with jobs metered against a plan.

Image detection (Path B), VR, payments, and smart recommendations are **out of MVP scope** and appear
under **Future Direction** (§F).
...
### F8.3 — Plan-limit enforcement
- [ ] Before any job is created, the backend checks `jobsUsedThisPeriod < plan.jobsPerMonth`.
- [ ] Over-limit requests return `403` with a clear "limit reached" message; usage counters increment
      atomically with job creation.
...
## F — Future Direction (outlined, post-MVP)

Each item below becomes a full epic (with acceptance criteria) when pulled into scope.

- **FB — Image Detection (Path B).** Upload a floor-plan image → CV detection → a `canvas` payload
  matching the same contract Path A produces, so it reuses the entire E4–E7 pipeline. Runs on GPU via a
  secure tunnel (see `03-deployment-and-scalability.md`). *Built only after the MVP backend works.*
- **FV — VR Pipeline.** Convert a scene to a VR-compatible format, reduce scene complexity, store the VR
  output, and deliver an interactive walkthrough on VR devices.
- **FP — Billing & Payments.** Payment provider integration (Stripe/Razorpay), subscription lifecycle,
  webhooks, invoices — layered on top of the plan/usage model already enforced in E8.
- **FA — Asset Management Backend.** Full CRUD for the asset library, categories, search, premium
  gating, and user custom-asset uploads to R2.
- **FR — Smart Asset Recommendations.** Context-aware suggestions using vector search (pgvector).
